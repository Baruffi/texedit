...
def updateKeys():
    ...
    if keys[pygame.K_BACKSPACE]:
        if cursor_x > 0:
            behind_cursor = canvas.getIdByPosition(
                (cursor_x - unit_size_x, cursor_y))
            if behind_cursor:
    ...
    if keys[pygame.K_DELETE]:
        if cursor_x < max_x:
            ahead_cursor = canvas.getIdByPosition(
                (cursor_x + unit_size_x, cursor_y))
            if ahead_cursor:
    ...
    for key in keys:
        if pygame.key.name(key) in character_dict.keys():
            cursor_pos = cursor_x, cursor_y
            below_cursor = canvas.getIdByPosition(cursor_pos)

            if below_cursor:
                canvas.updateSurface(
                    below_cursor, character_dict[pygame.key.name(key)])

                if cursor.getX() < max_x:
                    cursor.setPosition((cursor_x + unit_size_x, cursor_y))
                else:
                    cursor.setPosition((0, cursor_y + unit_size_y))
