...
def update():
    for event in pygame.event.get():
        ...
        if event.type == pygame.KEYDOWN:
            ...
            max_x = (screen.get_width() // unit_size_x) * unit_size_x
            ...
            if event.key == pygame.K_LEFT:
                if cursor_x - unit_size_x >= 0:
                    ...
                    cursor.setPosition(
                        (max_x - unit_size_x, cursor_y - unit_size_y))

            if event.key == pygame.K_RIGHT:
                if cursor_x + unit_size_x < max_x - unit_size_x:
                    cursor.setPosition((cursor_x + unit_size_x, cursor_y))
            ...
            if event.key == pygame.K_BACKSPACE:
                if cursor_x > 0:
                    ...
                    behind_cursor = canvas.getIdByPosition((max_x - unit_size_x, cursor_y - unit_size_y))
                    if behind_cursor:
                        canvas.getDrawable(behind_cursor).setSurface(empty_surface)
                        cursor.setPosition((max_x - unit_size_x, cursor_y - unit_size_y))

            if event.key == pygame.K_DELETE:
                if cursor_x < max_x - unit_size_x:
                    ...
                    ahead_cursor = canvas.getIdByPosition((0, cursor_y + unit_size_y))
                    if ahead_cursor:
                        canvas.getDrawable(ahead_cursor).setSurface(empty_surface)
                        cursor.setPosition((0, cursor_y + unit_size_y))
            ...
            if event.unicode in character_dict.keys():
                ...
                below_cursor = canvas.getIdByPosition(cursor_pos)

                if below_cursor:
                    canvas.updateSurface(
                        below_cursor, character_dict[event.unicode])

                    if cursor.getX() < max_x - unit_size_x:
                        cursor.setPosition((cursor_x + unit_size_x, cursor_y))
                    else:
                        cursor.setPosition((0, cursor_y + unit_size_y))

                cursor_pos = cursor.getPosition()
                below_cursor = canvas.getIdByPosition(cursor_pos)
                ...
                if not below_cursor:
                    ...
                    canvas.setDrawable(new_empty_zone, Drawable(
                        empty_surface, cursor.getX(), cursor.getY()))
